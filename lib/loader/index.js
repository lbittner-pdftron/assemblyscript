...
function postInstantiate(baseModule, instance) {
  const rawExports = instance.exports || instance; // ASM modules dont have `exports`, just use the instance instead
...
/** Instantiates an ASM module generated by AssemblyScript*/
function instantiateASM(module, imports) {
  return postInstantiate(
    preInstantiate(imports || (imports = {})),
    module
  );
...
exports.instantiateASM = instantiateASM;
...
function demangle(exports, baseModule) {
  ...
  for (let internalName in exports) {
    ...
    // asm files are generated with _'s instead of #'s
    // Maybe demangle could take a `delimiter` parameter instead?
    if (hash === -1) hash = name.indexOf('_'); 
    ...
    if (hash >= 0) {
      ...
      if (/^(get|set):/.test(name)) {
        if (!hasOwnProperty(curr, name = name.substring(4))) {
          let getter = exports[internalName.replace("set:", "get:").replace('set_', 'get_')]; // asm modules use '_' instead of ':'
          let setter = exports[internalName.replace("get:", "set:").replace('get_', 'set_')]; // again, maybe use `delimiter` here?
      ...
      if (/^(get|set):/.test(name)) {
        if (!hasOwnProperty(curr, name = name.substring(4))) {
          Object.defineProperty(curr, name, {
            get: exports[internalName.replace("set:", "get:").replace('set_', 'get_')], // asm modules use '_' instead of ':'
            set: exports[internalName.replace("get:", "set:").replace('get_', 'set_')], // again, maybe use `delimiter` here?
            enumerable: true
          });
        }
